...
function getToastProps(errorLogObj: ErrorLog) {
  ...
  const isChecksumMismatch = errorLogObj.message?.includes('checksum') &&
                            (errorLogObj.message?.includes('mismatch') ||
                             errorLogObj.message?.includes('expected'));

  if (isChecksumMismatch) {
    const props: ToastOptions = {
      message: t`Database Checksum Mismatch`,
      type: 'error',
    };
...
function getIssueUrlQuery(errorLogObj?: ErrorLog): string {
  const baseUrl = 'https://github.com/codeconomics-tz/accountinite/issues/new?labels=bug';
