    ...
    <template #body>
      ...
      >
        <div
          ...
          :key="key"
          class="relative text-sm cursor-pointer px-4 py-2.5 rounded-lg transition-all duration-300 ease-in-out overflow-hidden group transform"
          :class="
            key === activeTab
              ? 'bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-gray-25 font-semibold border-s-4 border-green-500 dark:border-green-400'
              : 'text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-gray-100 hover:bg-gray-50 dark:hover:bg-gray-850/80 hover:translate-x-1 hover:scale-[1.02]'
          "
          @click="activeTab = key"
        >
          ...
          <span 
            v-if="key !== activeTab"
            class="absolute inset-0 bg-gradient-to-r from-transparent to-gray-100 dark:to-gray-800 opacity-0 group-hover:opacity-30 transition-all duration-300 rounded-lg"
          ></span>
        </div>
      </div>
    </template>
...
export default defineComponent({
  ...
  methods: {
    ...
    async sync(): Promise<void> {
      ...
      this.update();
      await showDialog({
        title: this.t`Reload Accountinite?`,
