...
export async function getExcelExportData(
  ...
  for (const parentRow of parentData) {
    ...
    const tableFieldRowMap = parentNameMap[parentName];
    if (!tableFieldRowMap || !Object.keys(tableFieldRowMap ?? {}).length) {
      wsData.push([baseRowData, headers.child.map(() => '')].flat());
      continue;
    }
    ...
    for (const tableFieldName in tableFieldRowMap) {
      ...
      for (const tableRow of tableRows) {
        ...
        wsData.push([baseRowData, tableRowData].flat());
      }
    }
  }
  ...
  for (let col = range.s.c; col <= range.e.c; col++) {
    ...
    if (ws[cellAddress1]) {
      ws[cellAddress1].s = { font: { bold: true } };
    }
    if (ws[cellAddress2]) {
      ws[cellAddress2].s = { font: { bold: true } };
    }
  }
  ...
  const colWidths = wsData.reduce((widths, row) => {
    row.forEach((cell, colIndex) => {
      ...
      if (!widths[colIndex] || cellLength > widths[colIndex]) {
        widths[colIndex] = Math.min(cellLength + 2, 50);
      }
    });
    return widths;
  }, [] as number[]);
  ...
  XLSX.utils.book_append_sheet(wb, ws, schemaName);
