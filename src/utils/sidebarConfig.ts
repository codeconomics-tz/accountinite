...
function getCompleteSidebar(): SidebarConfig {
  const sidebarItems = [
    ...
    },
    {
      ...
      items: [
        ...
        {
          label: t`Atomic Profit And Loss`,
          name: 'atomic-profit-and-loss',
          route: '/report/AtomicProfitAndLoss',
        },
        {
  ...
  // Conditionally add other sections based on settings
  const inventorySection = getInventorySidebar();
  const posSection = getPOSSidebar();
  
  if (inventorySection.length > 0) {
    sidebarItems.push(...inventorySection);
  }
  
  if (posSection) {
    sidebarItems.push(posSection);
  }

  // Add Setup section last
  sidebarItems.push(setupSection);
