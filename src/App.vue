...
export default defineComponent({
  ...
  async mounted() {
    ...
    // Set up listener for app close event from main process
    if (ipc.registerAppCloseListener) {
      ipc.registerAppCloseListener(() => {
        this.handleAppClose();
      });
    }
  },
  methods: {
    ...
    async fileSelected(filePath: string): Promise<void> {
      ...
      if (filePath !== ':memory:' && !(await ipc.checkDbAccess(filePath))) {
        await showDialog({
          ...
          type: 'error',
          detail: this
            .t`Accountinite does not have access to the selected file: ${filePath}`,
        });
    ...
    async toggleTheme(): Promise<void> {
      ...
      const currentDarkMode = !!fyo.singles.SystemSettings?.darkMode;
      const newDarkMode = !currentDarkMode;
      
      // Update the SystemSettings document
      if (fyo.singles.SystemSettings) {
        await fyo.singles.SystemSettings.set('darkMode', newDarkMode);
        await fyo.singles.SystemSettings.sync();
      }
    ...
    async handleAppClose(): Promise<void> {
      // Check if we should show the thank you message based on random interval
      // Approximately 2-3 times per week
      const shouldShowMessage = Math.random() < 0.4; //40% Chance
      
      if (shouldShowMessage) {
        // Show the thank you message in-app using the dialog system
        await showDialog({
          title: 'Thank you for using Accountinite!',
          detail: 'Please do not forget to backup your accounts!',
          type: 'info',
        });
      ...
      // Tell the main process to allow the window to close
      if (ipc.allowWindowClose) {
        ipc.allowWindowClose();
      }
    },
  },
});
