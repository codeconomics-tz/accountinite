...
export default function getCommonExportActions(report: Report): Action[] {
  const exportExtention = ['csv', 'json', 'xlsx'] as ExportExtention[];
...
function getJsonData(report: Report): string {
  ...
  exportObject.softwareName = 'Accountinite';
...
function getXlsxData(report: Report): Uint8Array {
  const displayPrecision =
    (report.fyo.singles.SystemSettings?.displayPrecision as number) ?? 2;
  ...
  wsData.push(columns.map((c) => c.label)); // Add headers

  // Add data rows
  for (const row of reportData) {
    if (row.isEmpty) {
      wsData.push(Array(row.cells.length).fill(''));
      continue;
    }
  ...
  for (let c = 0; c <= range.e.c; c++) {
    ...
    colWidths.push({ wch: Math.min(maxWidth + 2, 50) }); // Cap at 50 characters max
  }

  (ws as XLSX.WorkSheet)['!cols'] = colWidths;

  XLSX.utils.book_append_sheet(wb, ws, report.reportName || 'Report');
