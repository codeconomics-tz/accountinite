...
export class GeneralLedger extends LedgerReport {
  ...
  // Optimized method for printing that can limit data processing
  async setPrintReportData(
    startIndex: number = 0,
    limitCount: number = 1000
  ): Promise<{ data: ReportData; totalCount: number }> {
    this.loading = true;
    
    // Fetch raw data with limit and offset for better performance
    if (this._rawData.length === 0) {
      await this._setRawData();
    }

    // Process only the required slice for printing
    const map = this._getGroupedMap(true);
    this._setIndexOnEntries(map);
    const { totalDebit, totalCredit } = this._getTotalsAndSetBalance(map);
    
    // For printing, we need to get a slice of the consolidated data
    const consolidated = this._consolidateEntries(map);
    ...
    const reportData = this._convertEntriesToReportData(consolidated);
    
    // Return only the requested slice plus total count for pagination
    const slicedData = startIndex >= 0 && limitCount > 0 
      ? reportData.slice(startIndex, startIndex + limitCount)
      : reportData;
      
    this.loading = false;
    return { data: slicedData, totalCount: reportData.length };
  }

  // Method to load data in chunks for progressive loading
  async loadChunk(startIndex: number, chunkSize: number = 100): Promise<ReportData> {
    // Use the existing setPrintReportData method but with specific parameters
    const result = await this.setPrintReportData(startIndex, chunkSize);
    return result.data;
  }
