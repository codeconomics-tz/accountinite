...
export default function registerIpcMainMessageListeners(main: Main) {
  ...
  ipcMain.on(IPC_MESSAGES.SHOW_NOTIFICATION, (_, message: string) => {
    // This is a fallback mechanism to show notifications in the renderer
    if (main.mainWindow) {
      main.mainWindow.webContents.send(IPC_MESSAGES.SHOW_NOTIFICATION, message);
    }
  });
}