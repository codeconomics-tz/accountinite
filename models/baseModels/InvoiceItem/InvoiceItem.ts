  ...
  formulas: FormulaMap = {
    ...
    rate: {
      formula: async (fieldname) => {
        ...
        const isItemDiscountedTotal = fieldname === 'itemDiscountedTotal';
...
function getDiscountedTotal(
  ...
  /**
   * Calculate discounted total using formulas
  ...
  if (setDiscountAmount) {
    ...
    return rate.mul(quantity).mul(1 - itemDiscountPercent / 100);
  }
  return rate.mul(quantity);
}
...
function getRate(
  ...
  itemDiscountAmount: Money,
  itemDiscountedTotal: Money,
  isItemDiscountedTotal: boolean,
  setItemDiscountAmount: boolean
) {
  if (isItemDiscountedTotal && setItemDiscountAmount) {
